...
import { mintNectar, nectarCreatedAt, nectarTimestampMs, isValidNectar } from "../dist/hive-graph/ulid.js";
import { sha256Hex } from "../dist/hive-graph/hash.js";
import { toRepoRelative, filenameOf, extOf } from "../dist/hive-graph/paths.js";
import {
  assertValidCatalogTable,
  HIVE_GRAPH_TABLE,
  HIVE_GRAPH_VERSIONS_TABLE,
  HIVE_GRAPH_CATALOG_GROUP,
} from "../dist/hive-graph/schema.js";
import { InMemoryHiveGraphStore } from "../dist/hive-graph/memory-store.js";
import { isValidEmbedding, EMBED_DIMS } from "../dist/hive-graph/model.js";
...
test("both catalog tables pass the load-time guard", () => {
  assert.doesNotThrow(() => assertValidCatalogTable(HIVE_GRAPH_TABLE));
  assert.doesNotThrow(() => assertValidCatalogTable(HIVE_GRAPH_VERSIONS_TABLE));
});

test("both tables are tenant-scoped and carry the tenancy columns", () => {
  for (const t of [HIVE_GRAPH_TABLE, HIVE_GRAPH_VERSIONS_TABLE]) {
...
test("every NOT NULL column carries a DEFAULT; embedding + confidence + fingerprint are nullable", () => {
  ...
  const versionCols = HIVE_GRAPH_VERSIONS_TABLE.columns;
...
test("hive_graph_versions carries a nullable fingerprint column (PRD-006 step-4 persistence) and still passes the guard", () => {
  const versionCols = HIVE_GRAPH_VERSIONS_TABLE.columns;
  ...
  assert.doesNotThrow(() => assertValidCatalogTable(HIVE_GRAPH_VERSIONS_TABLE), "the table still passes the load-time guard");
});

test("describe_status default is pending and skipped-deleted is a declared status", () => {
  const st = HIVE_GRAPH_VERSIONS_TABLE.columns.find((c) => c.name === "describe_status");
  assert.equal(st?.default, "pending");
});

test("the catalog group is named hive-graph with both tables", () => {
  assert.equal(HIVE_GRAPH_CATALOG_GROUP.name, "hive-graph");
  assert.equal(HIVE_GRAPH_CATALOG_GROUP.tables.length, 2);
});
