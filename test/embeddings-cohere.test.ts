/**
 * Cohere-via-Portkey embeddings transport tests (PRD-014b).
 *
 * Proves AC-2 (opt-in POSTs the Portkey embeddings endpoint with the
 * rerank-pattern headers; model id + output_dimension in the body; both
...
// ── AC-2: endpoint + rerank-pattern headers + model + output_dimension ─────────

test("014-AC-2 cohere opt-in POSTs the Portkey embeddings endpoint with the rerank-pattern headers", async () => {
  const seen: SeenRequest = { url: "", headers: {}, body: "" };
  const config = resolveEmbeddingsConfig({
    selector: "cohere",
    cohere: { apiKey: "pk-secret", configId: "pcfg-1" },
    env: {},
  });
...
test("014-AC-2 the body carries the default model id (embed-v4.0) and output_dimension (768)", async () => {
  const seen: SeenRequest = { url: "", headers: {}, body: "" };
  const config = resolveEmbeddingsConfig({
    selector: "cohere",
  ...
  assert.equal(body.model, DEFAULT_COHERE_EMBED_MODEL);
  assert.equal(body.model, "embed-v4.0");
  assert.equal(body.output_dimension, DEFAULT_COHERE_OUTPUT_DIMENSION);
  assert.equal(body.output_dimension, 768);
  assert.deepEqual(body.input, ["a", "b"]);
});

test("014-AC-2 model id and output_dimension are config-overridable (explicit overrides)", async () => {
  const seen: SeenRequest = { url: "", headers: {}, body: "" };
  const config = resolveEmbeddingsConfig({
    selector: "cohere",
    cohere: { apiKey: "k", configId: "c", model: "embed-multilingual-v4.0", outputDimension: 512 },
    env: {},
  });
  const provider = resolveEmbedProvider(config, { fetch: recordingFetch(seen) });
...
test("014-AC-2 model id and output_dimension are config-overridable (env)", () => {
  const config = resolveEmbeddingsConfig({
    selector: "cohere",
    env: {
      HIVENECTAR_EMBEDDINGS_COHERE_MODEL: "embed-v4.0-lite",
      HIVENECTAR_EMBEDDINGS_OUTPUT_DIMENSION: "1024",
      HIVENECTAR_PORTKEY_API_KEY: "envkey",
      HIVENECTAR_PORTKEY_CONFIG: "envcfg",
    },
  });
  assert.equal(config.cohere.model, "embed-v4.0-lite");
  assert.equal(config.cohere.outputDimension, 1024);
  assert.equal(config.cohere.apiKey, "envkey");
  assert.equal(config.cohere.configId, "envcfg");
});

// ── fail-soft + secret discipline (US-014b.3) ──────────────────────────────────

test("014-AC-2 cohere fails soft to nulls on a non-2xx and never surfaces the key", async () => {
  const fetch = async () => ({
    ok: false,
    status: 401,
  ...
  const config = resolveEmbeddingsConfig({
    selector: "cohere",
    cohere: { apiKey: "pk-secret", configId: "c", maxAttempts: 1 },
    env: {},
  });
...
test("014-AC-2 cohere fails soft to nulls on a network error (no throw)", async () => {
  const fetch = async () => {
    throw new Error("getaddrinfo ENOTFOUND api.portkey.ai");
  };
  const config = resolveEmbeddingsConfig({
    selector: "cohere",
    cohere: { apiKey: "k", configId: "c", maxAttempts: 1 },
    env: {},
  });
  const provider = resolveEmbedProvider(config, { fetch, sleep: async () => undefined });
...
test("014-AC-2 cohere retries a 429 within the bound, then succeeds", async () => {
  let calls = 0;
  const fetch = async () => {
    calls += 1;
...
test("014-AC-2 cohere not keyed fails soft to nulls without a POST", async () => {
  let called = false;
  const fetch = async () => {
    called = true;
    return { ok: true, status: 200, text: async () => "{}" };
  };
  const config = resolveEmbeddingsConfig({ selector: "cohere", env: {} });
  assert.equal(config.cohere.apiKey, null);
