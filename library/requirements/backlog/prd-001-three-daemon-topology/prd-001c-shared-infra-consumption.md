...
This sub-PRD defines the **shared-infra consumption contract**: how hivenectar reaches the four pieces of shared infrastructure — the **Portkey gateway**, the **embeddings daemon**, **CodeGraph**, and the **recall engine** — through its **own** clients over the network, never in-process. It is the answer to ADR-0002's framing that independence changes *how* hivenectar reaches shared infra (its own client, its own auth) but not *whether* it uses it. hivenectar consumes the same Portkey endpoint, the same embeddings daemon, the same CodeGraph service, and writes into the same recall-readable tables as honeycomb — but each seam is crossed over HTTP from hivenectar's own process.

The load-bearing invariant this PRD establishes is the **deploy-time tenancy invariant**: hivenectar's rows must be readable by honeycomb's recall engine under the same `org`/`workspace`/`project` scope. ADR-0002 names this as a negative consequence — a misconfiguration (hivenectar pointing at a different Deep Lake org) silently breaks recall integration. This PRD states the invariant; PRD-005 owns the tenancy model and PRD-013 owns the recall arm that reads the rows.

## Goals

- Name each of the four shared-infra seams and state that hivenectar reaches each through its **own** client over the network (no in-process import of honeycomb's runtime).
- Specify the **Portkey** consumption seam: same gateway URL, same `x-portkey-api-key` + `x-portkey-config` header pair, reached from hivenectar's own inference transport.
- Specify the **embeddings** consumption seam: same nomic-embed-text-v1.5 daemon at the same loopback URL, same 768-dim contract, reached from hivenectar's own embed client.
- Specify the **CodeGraph** consumption seam: hivenectar reuses the same discovery/build patterns, not the in-process module.
- Specify the **recall** seam: hivenectar does NOT ship its own recall engine; it composes by writing `source_graph_versions` rows that honeycomb's shared recall engine reads (PRD-013 adds the arm).
...
- The recall arm that reads `source_graph_versions` — **PRD-013**.
- The harness-exposure documentation (no hivenectar-shipped hooks) — **PRD-009**.
- The Deep Lake table schemas + tenancy model — **PRD-005**. This PRD consumes the tenancy as an invariant; it does not define it.
- hivenectar's Deep Lake client itself — [`prd-001b`](./prd-001b-hivenectar-process-and-health.md). This PRD covers the *other* shared infra (Portkey, embeddings, CodeGraph, recall), not Deep Lake.

---

## The four seams

Each seam is crossed over the network from hivenectar's own process. None is imported from honeycomb's in-process runtime.
...
The composition above works only if hivenectar's rows are readable by honeycomb's recall engine under the same scope. This is the **deploy-time invariant** ADR-0002 negative consequence #2 names:

> Both daemons point at the same Deep Lake datasets with compatible tenancy. A misconfiguration (hivenectar pointing at a different Deep Lake org than honeycomb) silently breaks recall integration. This was impossible under colocation; it is now a deploy-time invariant to enforce.

Concretely, hivenectar's Deep Lake client (PRD-001b) and honeycomb's Deep Lake client must resolve to the **same `org`** and operate in the **same `workspace`**, with `project_id` as the shared column filter. hivenectar's installer/bootstrap must verify the resolved org matches honeycomb's at first run; a mismatch is a configuration error, not a runtime failure to swallow silently.

> **DEFAULT — confirm before implementation.** The exact verification mechanism (a bootstrap-time org-equality check, a shared config file, a hivedoctor-mediated assertion) is a PRD-002/PRD-003 operations decision. The contract here is only: the invariant exists, and a violation silently breaks recall — it must be enforced, not assumed.

---

## User stories

### US-001c.1 — hivenectar routes LLM calls through Portkey from its own client
**As** hivenectar, **when** I generate a description, **I** POST to the Portkey chat-completions URL with the `x-portkey-api-key` + `x-portkey-config` headers from my own transport, **so that** I route through the same gateway honeycomb uses without importing its process.

- Acceptance: hivenectar's inference transport targets `https://api.portkey.ai/v1/chat/completions` with the fixed header pair.
- Acceptance: hivenectar does not import honeycomb's `createPortkeyTransport` in-process.

### US-001c.2 — hivenectar embeds through the shared embeddings daemon
**As** hivenectar, **when** I embed a description, **I** POST to `http://127.0.0.1:3851/embed` and receive a 768-dim vector, **so that** the embedding matches the schema's `FLOAT4[]` columns.

- Acceptance: hivenectar's embed client targets the shared daemon at 3851 and honors the 768-dim contract.

### US-001c.3 — hivenectar composes with recall by writing rows, not shipping an engine
**As** an agent, **when** I run a fused recall query through honeycomb, **I** see hivenectar's `source_graph_versions` hits alongside session/memory hits, **so that** one query returns both semantic-file and conversation-trace results.

- Acceptance: hivenectar writes `source_graph_versions` rows under the shared `org`/`workspace`/`project` tenancy.
- Acceptance: honeycomb's recall engine (with PRD-013's arm) reads those rows; hivenectar ships no recall engine of its own.

### US-001c.4 — A tenancy mismatch is caught, not silently swallowed
**As an** operator, **when** hivenectar's resolved org differs from honeycomb's, **the** bootstrap surfaces the mismatch, **so that** recall integration is not silently broken.

- Acceptance: hivenectar verifies its resolved Deep Lake org matches honeycomb's at deploy/bootstrap time (mechanism is a PRD-002/003 default).
