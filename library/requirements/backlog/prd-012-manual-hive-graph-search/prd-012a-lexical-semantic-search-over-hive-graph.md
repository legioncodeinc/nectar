...
- Adding `hive_graph_versions` to the **fused agent-facing recall** — **PRD-013**. This engine is standalone.
...
): Promise<{ hits: HiveGraphHit[]; sources: ("nectar")[]; degraded: boolean }>;
```
...
The `source` projection is the literal `'nectar'`, carried from the integration spec ([`knowledge/private/data/recall-integration.md`](../../../knowledge/private/data/recall-integration.md)). The projected columns (`id`, `path`, `title`, `body`, `concepts`, `content_hash`) match the engine's `HiveGraphHit` shape.
...
**Acceptance criteria:**
- AC-012a.4.1 Given `hive_graph_versions` does not exist, then the engine returns `{ hits: [], sources: [], degraded: true }`, never a 500, mirroring the per-arm fail-soft at `honeycomb/src/daemon/runtime/memories/recall.ts:24-35`.
...
- [`../../../knowledge/private/data/hive-graph-schema.md`](../../../knowledge/private/data/hive-graph-schema.md) — the `hive_graph_versions` columns (`title`, `description`, `concepts`, `embedding`, `describe_status`, `seq`, `nectar`, tenancy).
