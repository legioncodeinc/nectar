...
- Reuse the **per-arm fail-soft** pattern: a missing `source_graph_versions` table (fresh workspace) degrades to an empty result, never a 500, mirroring the recall engine's guarded-query discipline (`honeycomb/src/daemon/runtime/memories/recall.ts:24-35`).
...
- [ ] A missing `source_graph_versions` table (fresh workspace) degrades to `{ hits: [], sources: [], degraded: true }`, never a 500, mirroring the per-arm fail-soft at `honeycomb/src/daemon/runtime/memories/recall.ts:24-35`.
...
- [`prd-005-source-graph-catalog-tables`](../../completed/prd-005-source-graph-catalog-tables/prd-005-source-graph-catalog-tables-index.md) — owns the tables this engine reads.
