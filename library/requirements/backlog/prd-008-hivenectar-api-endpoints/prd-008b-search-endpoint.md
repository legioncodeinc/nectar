...
- Mount `POST /api/source-graph/search` (and/or `GET` with a query parameter) as a thin handler on the group 008a scaffolds, modeled on `mountGraphApi`'s attach shape (`honeycomb/src/daemon/runtime/codebase/api.ts:304-347`).
...
The handler attaches to the `/api/source-graph` group and registers at the path relative to the group (`/search`). It resolves scope per-request (mirroring `honeycomb/src/daemon/runtime/codebase/api.ts:309-310`), delegates to PRD-012's engine, and surfaces failure as data.
...
The `options.searchSourceGraph` dependency is PRD-012's engine, injected at daemon construction (the same injection shape `mountGraphApi` uses for `options` — `honeycomb/src/daemon/runtime/codebase/api.ts:304`). The endpoint never reaches storage directly; it calls the engine, which in turn reads `source_graph_versions` through the injected storage client (`honeycomb/src/daemon/runtime/server.ts:13-16` FR-6).
...
- **Inject the engine, do not reach storage.** The handler takes `options.searchSourceGraph` (PRD-012's engine) and calls it; it never issues a storage query itself (the daemon is the only DeepLake client — `honeycomb/src/daemon/runtime/server.ts:13-16` FR-6).
