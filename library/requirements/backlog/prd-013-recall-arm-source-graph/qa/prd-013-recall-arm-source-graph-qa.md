# QA Report: PRD-013 Recall Arm — `source_graph_versions` (PRD-vs-Corpus-vs-Code Conformance)

> Category: QA Report | Version: 1.0 | Date: July 2026 | Status: Active

Wave-0 spec-QA gate audit of PRD-013 (index + 013a/013b/013c) against the Hivenectar knowledge corpus and the real Honeycomb code it cites, armed with quality-stinger + hivenectar-stinger. This is a PRD-vs-corpus/code pass — PRD-013's implementation lands out-of-band in the `honeycomb` repo (it extends honeycomb's fused recall); no honeycomb code was modified by this audit. Matches the bar and format of [`prd-005-source-graph-catalog-tables-qa.md`](../../../completed/prd-005-source-graph-catalog-tables/qa/prd-005-source-graph-catalog-tables-qa.md). Every acceptance criterion and load-bearing claim was traced to [`data/recall-integration.md`](../../../../knowledge/private/data/recall-integration.md) (the authoritative integration spec), [`PRD-DECISIONS-AND-DEFAULTS.md`](../../../PRD-DECISIONS-AND-DEFAULTS.md) decision #17, [`PRD-003-016-DEPENDENCY-MAP.md`](../../../PRD-003-016-DEPENDENCY-MAP.md), and the real files under `honeycomb/src/daemon/`.

**Related:**
- [`prd-013-recall-arm-source-graph-index.md`](../prd-013-recall-arm-source-graph-index.md)
...
### W-1 (Correctness, 013a — REPORTED, not fixed): Decision #17's operator-tunable `hivenectar_rrf_multiplier` has no AC, design, or code path anywhere

[`PRD-DECISIONS-AND-DEFAULTS.md`](../../../PRD-DECISIONS-AND-DEFAULTS.md) decision #17 (locked) reads: *"Recall arm weight: `ARM_CLASS_WEIGHT` for source_graph_versions = 1.0 (peer with distilled memory); operator-tunable via `hivenectar_rrf_multiplier` at runtime."* PRD-013a's "Weight `1.0`" section (`prd-013a-lexical-arm-builder-and-weight.md:87-89`) narrates the second half of that decision as if it already exists: *"an operator who finds Hivenectar hits dominating recall at the expense of session memory lowers the class via `~/.honeycomb/hivenectar.json` (`recall.hivenectar_rrf_multiplier`)."* This mechanism does not exist:

- A repo-wide search of `honeycomb/src` for `hivenectar_rrf_multiplier` and `hivenectar.json` returns **zero** matches — there is no code anywhere that reads such a key.
...
|---|---|---|
| `prd-013-recall-arm-source-graph-index.md` | 6 | 71-76 |
...
Locations (now fixed): `prd-013-recall-arm-source-graph-index.md:23`; `prd-013a-lexical-arm-builder-and-weight.md:149, 199`.
...
|---|---|---|
| `"source_graph_versions"` is a member of `RecallSource` (`recall.ts:169`); `readSource` recognizes it | `recall.ts:169` (exact), `:385-389` (exact) | PASS |
| `buildSourceGraphVersionsArmSql` mirrors `buildMemoriesArmSql` | `recall.ts:319-337` (exact) | PASS |
...
| Every column exists in the PRD-005b DDL | `prd-005b-source-graph-versions-table.md:35-58` (verbatim DDL) | PASS — `nectar`, `seq`, `title`, `description`, `concepts`, `describe_status`, `described_at` all present |
| `kindOfSource("source_graph_versions")` → `"memory"`, no code change needed; RRF contribution `1.0/(60+rank)` | `recall.ts:141` (`RRF_K`), `:158-161`, `:164-166` (all exact) | PASS; weight value correct, tunability mechanism gap is W-1 |
...
- **PRD-009 slug (corrected 2026-07-02):** confirmed. `prd-013-recall-arm-source-graph-index.md:70` links `../prd-009-harness-exposure-via-recall/prd-009-harness-exposure-via-recall-index.md`, which resolves to the real folder (`backlog/prd-009-harness-exposure-via-recall/`, confirmed on disk). `PRD-003-016-DEPENDENCY-MAP.md` D-8 independently confirms this correction as RESOLVED.
...
- `prd-013-recall-arm-source-graph-index.md` — audited and remediated (W-2, W-3, W-5).
...
All fixes were applied in place inside `prd-013-recall-arm-source-graph/`; no corpus, no other PRD folder, and no honeycomb file was touched.
...
| W-4 | Medium | 8 stale `../prd-005-source-graph-catalog-tables/...` and `../prd-014-embeddings-provider-switching/...` sibling links corrected to `../../completed/...` (6×, PRD-005b/c) and `../../in-work/...` (2×, PRD-014) across 013a/013b/013c. |
...
| S-1 | Sub-medium | Removed the unused `pathCol` declaration from the `buildSourceGraphVersionsArmSql` code sketch in 013a; the SQL output is unchanged. |
| W-1 | Medium | **Not fixed — reported.** Requires a PRD-authoring decision on how (or whether) to scope the `hivenectar_rrf_multiplier` mechanism; recorded for `library-worker-bee` / the PRD author. |
...
**Verification:** `grep -rn "\.\./\.\./\.\./MASTER-PRD-INDEX"`, `grep -rnE "\]\([./]*honeycomb[^)]*\)"`, `grep -rnE "\]\(\.\./prd-005-source-graph-catalog-tables|\.\./prd-014-embeddings-provider-switching[^)]*\)"`, `grep -rn "128-129"`, and `grep -rn "pathCol"` all return zero across the four PRD-013 files post-remediation.
