...
Brooding is **resumable**. A brood can be killed mid-flight (laptop closed, process crashed, operator Ctrl-C) and the next boot picks up exactly where it left off, with no lost work and no duplicate description cost. The state of the brood is **fully derivable** from the `hive_graph_versions.describe_status` column — there is no "brood in progress" lockfile and no partial-state marker.
...
The three rules (skip already-brooded / re-enqueue minted-not-described / discover fresh) and the absence of a lockfile are the entire state machine. There are no transitions to invent — the `describe_status` column ([PRD-005b](../../completed/prd-005-hive-graph-catalog-tables/prd-005b-hive-graph-versions-table.md)) is the single source of truth, and the brooder's resumption logic is a read over it.
...
- The `describe_status` column's DDL and its full value set — [PRD-005b](../../completed/prd-005-hive-graph-catalog-tables/prd-005b-hive-graph-versions-table.md). This sub-PRD *reads* the column; PRD-005 owns its definition.
...
- The single-instance PID/lock that prevents two daemons — [PRD-002d](../../completed/prd-002-nectar-daemon/prd-002d-single-instance-lock-and-shutdown.md). That lock is about the *process*, not the *brood* — a brood has no lock of its own.
...
On a resumed brood (next boot after a kill, or any brood invoked against a project that already has `hive_graph` / `hive_graph_versions` rows), the brooder applies three rules, carried verbatim from [`brooding-pipeline.md`](../../../knowledge/private/ai/brooding-pipeline.md):

```mermaid
flowchart TD
    boot["brood invoked / daemon boots into brood mode"] --> scan["scan hive_graph_versions.describe_status"]
...
A file whose nectar was minted (the `hive_graph` identity row exists) and whose version row exists but is still `pending` was mid-bucket when the kill happened. On resume it is re-enqueued into the bucketing flow ([007b](./prd-007b-bucketing-and-llm-call-shapes.md)) — it is *not* re-minted (the nectar is stable) and it is *not* skipped (no description was committed).

### Rule 3 — Discover fresh

> "Files not yet minted are discovered fresh and enter the bucketing flow."

A file with no `hive_graph` identity row at all was never reached by the killed brood. On resume it is discovered fresh ([007a](./prd-007a-discovery-and-content-hash-precheck.md)) and enters bucketing as if for the first time.

---

## `describe_status` as the single discriminator

The whole state machine keys off one column: `hive_graph_versions.describe_status` ([PRD-005b](../../completed/prd-005-hive-graph-catalog-tables/prd-005b-hive-graph-versions-table.md)). Its values and what a resumed brood does with each:
...
- The resumption read is a scan over `hive_graph_versions` filtered by `describe_status` + tenancy (`org_id`/`workspace_id`/`project_id`, per [PRD-005c](../../completed/prd-005-hive-graph-catalog-tables/prd-005c-tenancy-and-project-id-filter.md)). The brooder joins to `hive_graph` only to distinguish "minted but pending" (rule 2) from "not minted at all" (rule 3) — i.e. a nectar exists in `hive_graph` but its latest version is `pending`, versus no nectar exists.
- "Latest version per nectar" is `ORDER BY seq DESC LIMIT 1` ([PRD-005b](../../completed/prd-005-hive-graph-catalog-tables/prd-005b-hive-graph-versions-table.md) — the `seq` monotonic counter), so the resume scan keys off the *latest* version row's status, not any historical one.
...
- [PRD-005b](../../completed/prd-005-hive-graph-catalog-tables/prd-005b-hive-graph-versions-table.md) — the `describe_status` column + `seq` counter this state machine reads.
- [PRD-005c](../../completed/prd-005-hive-graph-catalog-tables/prd-005c-tenancy-and-project-id-filter.md) — the tenancy scope of the resume scan.
...
- [PRD-002d](../../completed/prd-002-nectar-daemon/prd-002d-single-instance-lock-and-shutdown.md) — the *process* lock (distinct from the absent *brood* lock) + the graceful-shutdown path that triggers projection regen.
