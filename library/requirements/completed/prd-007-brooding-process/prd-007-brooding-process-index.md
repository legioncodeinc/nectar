...
> **Priority:** P0
> **Effort:** L
> **Schema changes:** None (PRD-007 owns no Deep Lake table — it *writes* `hive_graph` + `hive_graph_versions` owned by [PRD-005](../../completed/prd-005-hive-graph-catalog-tables/prd-005-hive-graph-catalog-tables-index.md). This PRD owns the one-time bootstrap pipeline.)
...
- Specify the **resumability state machine** via `hive_graph_versions.describe_status` (no lockfile, no partial-state marker), confirming the append-only/resumable pattern Honeycomb uses for the pollinating loop and skillify miner.
...
- The tables brooding writes (`hive_graph`, `hive_graph_versions`) and their DDL — [PRD-005](../../completed/prd-005-hive-graph-catalog-tables/prd-005-hive-graph-catalog-tables-index.md). This PRD *writes* those tables; PRD-005 owns the schema.
...
- The daemon process, worker harness, and the CLI *invocation* dispatch — [PRD-002](../../completed/prd-002-nectar-daemon/prd-002-nectar-daemon-index.md) (the worker in [002b](../../completed/prd-002-nectar-daemon/prd-002b-hiveantennae-worker.md), the CLI surface in [002c](../../completed/prd-002-nectar-daemon/prd-002c-nectar-cli-surface.md)). This PRD owns the brooding *mechanics* those surfaces invoke.
- The daemon's HTTP API endpoints (`/api/hive-graph/*`, including the `POST /api/hive-graph/build` trigger that maps to brooding) — [PRD-008](../prd-008-nectar-api-endpoints/prd-008-nectar-api-endpoints-index.md).
...
- [ ] Brooding is resumable via `hive_graph_versions.describe_status` with **no lockfile and no partial-state marker**; a killed-mid-brood daemon resumes on the next boot using the three skip / re-enqueue / discover-fresh rules.
...
- [`knowledge/private/data/hive-graph-schema.md`](../../../knowledge/private/data/hive-graph-schema.md) — the tables brooding writes.
...
- [`prd-002-nectar-daemon`](../../completed/prd-002-nectar-daemon/prd-002-nectar-daemon-index.md) — the daemon process + the worker ([002b](../../completed/prd-002-nectar-daemon/prd-002b-hiveantennae-worker.md)) that drives brooding + the CLI surface ([002c](../../completed/prd-002-nectar-daemon/prd-002c-nectar-cli-surface.md)) that invokes it.
- [`prd-005-hive-graph-catalog-tables`](../../completed/prd-005-hive-graph-catalog-tables/prd-005-hive-graph-catalog-tables-index.md) — the tables brooding writes ([005b](../../completed/prd-005-hive-graph-catalog-tables/prd-005b-hive-graph-versions-table.md) `describe_status` is the resumability key).
