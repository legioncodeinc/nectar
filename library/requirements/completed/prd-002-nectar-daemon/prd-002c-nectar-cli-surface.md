# PRD-002c: Nectar CLI Surface

> Parent: [`prd-002-nectar-daemon-index.md`](./prd-002-nectar-daemon-index.md)

## Overview

This sub-PRD documents the **CLI surface** an operator uses to drive the nectar daemon. It is a documentation PRD for the command *invocations*; the *mechanics* each command invokes are owned by sibling PRDs (brooding → PRD-007, projection regen → PRD-011, pruning/review → PRD-006, model swap → PRD-010). The surface has two entry binaries: the bare **`nectar`** binary (owns `daemon`, the lifecycle verb) and the **`honeycomb nectar <verb>`** sub-command namespace (owns the operational verbs that mint/prune/review/rebuild). The split mirrors how honeycomb separates the daemon lifecycle verb (`daemon start|stop|status`, `honeycomb/src/cli/index.ts`) from the storage/operational verbs that reach the daemon over loopback.
...
- Specify the **two entry binaries** (`nectar` lifecycle vs `honeycomb nectar <verb>` operational) and confirm they mirror honeycomb's lifecycle/operational split.
...
- The daemon bootstrap the `daemon` verb invokes — [`prd-002a`](./prd-002a-nectar-bootstrap-and-composition-root.md). This PRD documents the verb.
- The worker the commands enqueue into — [`prd-002b`](./prd-002b-hiveantennae-worker.md).
- The HTTP API endpoints (`/api/hive-graph/*`) — PRD-008. The CLI verbs and the API endpoints are parallel surfaces over the same mechanics.
...
```mermaid
flowchart LR
    subgraph bare["nectar binary (lifecycle)"]
        daemon["nectar daemon"]
    end
    subgraph ns["honeycomb nectar namespace (operational)"]
...
| Entry | Owns | Mirrors |
|---|---|---|
| `nectar` (bare binary) | The `daemon` lifecycle verb — invokes the composition root (002a) directly | honeycomb's daemon lifecycle verb (`honeycomb/src/cli/index.ts` "daemon start|stop|status drives the daemon lifecycle") |
| `honeycomb nectar <verb>` (sub-command namespace) | The operational verbs (brood/prune/review/rebuild) — reach the running daemon over loopback `:3854` | honeycomb's storage verbs reaching the daemon over the loopback `DaemonClient` (`honeycomb/src/cli/index.ts` "storage verbs reach the daemon through the real loopback DaemonClient") |
...
**Mechanic owner:** [`prd-002a`](./prd-002a-nectar-bootstrap-and-composition-root.md) (bootstrap) + [`prd-002d`](./prd-002d-single-instance-lock-and-shutdown.md) (lock + shutdown).

**Named in corpus:** the runnable process is "`nectar daemon`" ([`MASTER-PRD-INDEX.md`](../../MASTER-PRD-INDEX.md) CLI table) — "the runnable process."
...
**What it does:** Triggers a full brood — discovers files, bucketing by size/type, batched LLM description, writes `hive_graph` + `hive_graph_versions` rows, embeds, regenerates `.honeycomb/nectars.json`. Respects existing descriptions (only `pending`/undescribed files are described).

**Mechanic owner:** PRD-007 (the brooding pipeline).

**Named in corpus:** [`knowledge/private/ai/brooding-pipeline.md`](../../../knowledge/private/ai/brooding-pipeline.md) "Triggering brooding" — `honeycomb nectar brood # full brood, respects existing descriptions`.
...
**Named in corpus:** [`knowledge/private/ai/brooding-pipeline.md`](../../../knowledge/private/ai/brooding-pipeline.md) — `honeycomb nectar brood --force # re-describe every file, ignore existing`; [`knowledge/private/ai/enricher-and-llm-model.md`](../../../knowledge/private/ai/enricher-and-llm-model.md) "An operator who swaps models and wants to re-describe everything runs `honeycomb nectar brood --force --model <new>`."
...
**Named in corpus:** [`knowledge/private/ai/enricher-and-llm-model.md`](../../../knowledge/private/ai/enricher-and-llm-model.md) — "runs `honeycomb nectar brood --force --model <new>`, which sets all non-skipped rows back to `pending`."
...
**Named in corpus:** [`knowledge/private/ai/brooding-pipeline.md`](../../../knowledge/private/ai/brooding-pipeline.md) — `honeycomb nectar brood --limit 100 # brood at most 100 pending files (cost cap)`.
...
**Named in corpus:** [`knowledge/private/ai/brooding-pipeline.md`](../../../knowledge/private/ai/brooding-pipeline.md) — `honeycomb nectar brood --dry-run # show buckets and cost estimate, no LLM calls`; "It is the recommended first step on any new project to sanity-check the cost before committing to it."

---

### `honeycomb nectar prune --confirm`

**Invocation:** `honeycomb nectar prune --confirm`
...
**Named in corpus:** [`knowledge/private/ai/identity-and-reassociation.md`](../../../knowledge/private/ai/identity-and-reassociation.md) step 4 — "it surfaces the candidate match to the dashboard (or to an interactive `honeycomb nectar review-matches` command) for human confirmation."
...
### `honeycomb nectar rebuild-projection`

**Invocation:** `honeycomb nectar rebuild-projection`
...
**Named in corpus:** [`MASTER-PRD-INDEX.md`](../../MASTER-PRD-INDEX.md) CLI table — `honeycomb nectar rebuild-projection # explicit projection regen`; [`knowledge/private/data/portable-registry.md`](../../../knowledge/private/data/portable-registry.md) (the projection's three generation triggers).

---

### `honeycomb nectar project --rebuild-projection`

**Invocation:** `honeycomb nectar project --rebuild-projection`
...
**Named in corpus:** [`MASTER-PRD-INDEX.md`](../../MASTER-PRD-INDEX.md) CLI table — `honeycomb nectar project --rebuild-projection # project-scoped variant`.
...
**As an** operator, **when** I run `nectar daemon`, **the** daemon boots, binds `127.0.0.1:3854`, and serves `/health`, **so that** doctor can supervise it.

- Acceptance: `nectar daemon` invokes the composition root (002a) and is the only command that opens a socket.
...
**As an** operator, **when** I run `honeycomb nectar brood --dry-run`, **I** see the bucket counts and cost estimate with no LLM calls, **so that** I can sanity-check cost first.
...
**As an** operator, **when** I swap models, **I** run `honeycomb nectar brood --force --model <new>`, **so that** every non-skipped file is re-described by the new model.
...
**As an** operator, **when** I run `honeycomb nectar brood --limit 100`, **only** 100 pending files are brooded, **so that** I bound the spend.
...
**As an** operator, **when** I run `honeycomb nectar prune --confirm`, **nectars** missing past the 30-day grace period are removed, **so that** the history chain is not polluted by long-gone files.
...
**As an** operator, **when** I run `honeycomb nectar review-matches`, **I** see the low-confidence TLSH candidates, **so that** a mis-association does not corrupt the history chain.
...
**As an** operator, **when** I run `honeycomb nectar rebuild-projection`, **the** projection regenerates from Deep Lake, **so that** the committed `.honeycomb/nectars.json` matches the source of truth.
