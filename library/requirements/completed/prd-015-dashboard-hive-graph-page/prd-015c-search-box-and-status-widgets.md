...
Both are scoped to the dashboard-selected project (no cross-project leakage, matching 015a/015b) and both degrade to an unreachable state when nectar is down — hive's always-on shell stays up regardless ([PRD-004c](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md) c-AC-2/c-AC-6).
...
| ID | Criterion |
|---|---|
| c-AC-1 | Given the search box, when the operator submits a query, then it calls `wire.hiveGraphSearch(query, project)` — routed by hive's aggregation `wire` to nectar's `/api/hive-graph/search` (PRD-008/PRD-012), mirroring the `recall(query, project)` shape (`wire.ts:1485`). |
...
| c-AC-3 | Given a selected result, when the operator clicks it, then the file graph's search-to-node focus hook fires (015b's `findNode` → `setSelected` → `centerOn`, `graph.tsx:497-509`) so the canvas centers on that nectar. |
...
| c-AC-6 | Given the `usePoll` lifecycle (015a), when the interval fires, then the widgets refresh their counts without a manual reload, paused in background tabs (`isTabHidden()`, `graph.tsx:459`). |
| c-AC-7 | Given the status fetch fails (nectar unreachable), when the `wire` call degrades, then the widgets show the source as unreachable (hive fail-soft, [PRD-004c](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md) c-AC-6), not stale or zeroed numbers. |
...
| ID | Criterion |
|---|---|
| c-AC-8 | Given a build action on the page, when the operator invokes it, then it calls `wire.hiveGraphBuild()` — routed to nectar's `/api/hive-graph/build` (PRD-008), modeled on honeycomb's `buildGraph()` (`wire.ts:1614`) — and acknowledges the trigger; the status widgets reflect the new queue depth on the next poll. |
...
The page reads the shared `PageProps.wire` (015a) — the same-origin client whose requests hive's server-side proxy federates (hive ADR-0002). Three new methods mirror existing shapes:

| New `wire` method | Mirrors | Routes to (PRD-008) |
|---|---|---|
| `hiveGraphSearch(query, project)` | `recall(query, project)` (`wire.ts:1485`) | `/api/hive-graph/search` |
| `hiveGraphStatus(project)` | the project-scoped status reads (e.g. `kpis(project)`, `wire.ts:1448`) | `/api/hive-graph/status` |
| `hiveGraphBuild()` | `buildGraph()` (`wire.ts:1614`) | `/api/hive-graph/build` |

Each is an addition to the shared `wire` (PRD-004c's client), fetching hive's own origin same-origin; hive's SERVER-side proxy (`hive/src/daemon/proxy.ts`, hive ADR-0002) maps the request to nectar's host (its registry entry's `healthUrl` host, where `/api/*` lives) over loopback. No new nectar route is invented here; the endpoints are PRD-008's.

### Search box → graph focus

The search box's selected-result handler calls into the file graph's focus hook. `GraphPage` already factors this as `onSearch` → `findNode(rendered, raw)` → `setSelected` → `centerOn` (`graph.tsx:497-509`). `HiveGraphPage` lifts the same `selected`/`setSelected` + `transform`/`setTransform` state 015a establishes and exposes a focus callback the search box invokes on result click (c-AC-3). The search box renders its own results list (nectar + path + description); the canvas focus is the cross-feature seam.
...
- [`prd-012-manual-hive-graph-search`](../../in-work/prd-012-manual-hive-graph-search/prd-012-manual-hive-graph-search-index.md) — owns the search semantics the search box consumes.
- [`prd-007-brooding-process`](../../in-work/prd-007-brooding-process/prd-007-brooding-process-index.md) — owns the brooding pipeline the `/build` trigger launches + the cost math the counter surfaces.
- [`prd-004c-hive-portal-daemon`](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md) — owns hive's aggregation `wire` these fetches route through.

No open questions. The widget layout (panel order / sizing) follows the existing dashboard panel rhythm (`Panel` / `PageFrame`, no new design token — D-3/D-9 in `page-frame.tsx`); the exact field set the `/status` payload returns is owned by PRD-008, not pinned here.
