...
> **Priority:** P1
> **Effort:** M (3-8h)
> **Schema changes:** None (nectar) — the page consumes existing/new nectar API endpoints (PRD-008); it adds no Deep Lake table. The route registry + page component land in hive (a first-class product in its own `hive` repo, per PRD-004c + hive ADR-0001).
...
Two facts from the dashboard code fix the design:

1. **The dashboard now lives in hive, not the honeycomb daemon.** Per [decision #1](../../MASTER-PRD-INDEX.md) and [PRD-004c](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md), hive is the single source of always-on UI truth — it boots on OS start, owns the dashboard code (copied from honeycomb and retired there, per hive ADR-0001; it lives at `hive/src/dashboard/web/`), and fetches each page's data from the owning daemon's `/api/*` through a server-side proxy (hive ADR-0002). So this page's `RouteEntry` lands in the `ROUTES` list hive owns, and its data fetches route through hive's proxy to nectar's `/api/hive-graph/*` endpoints (PRD-008).

2. **A new page, not a third graph on `/graph`.** The dashboard already had a codebase-graph view that was **removed for being too dense to be useful** (`hive/src/dashboard/web/registry.tsx:215-218`); `/graph` now renders ONLY the memory/knowledge graph. Adding the Hive Graph as another graph on `/graph` repeats that density failure. The page is therefore its own route, `/hive-graph`, with its own kind filter and search so the file-provenance view never competes for canvas with the memory graph.
...
- Add ONE `RouteEntry` to the shared `ROUTES` list (`registry.tsx:204-231`) for `/hive-graph`, so the page appears in hive's sidebar + router outlet with no sidebar/router hand-edit (the 037c contract).
- Ship ONE page component (`HiveGraphPage`) taking the shared `PageProps` (`registry.tsx:92`) and hydrating via `usePoll`/`wire` — mirroring the existing `GraphPage` data-fetch pattern (`graph.tsx:435-482`).
...
- Keep the page honest under the always-on posture: when nectar is down, hive renders the shell + this page's source as unreachable rather than blanking the dashboard (c-AC-2 from [PRD-004c](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md)).

## Non-Goals

- The nectar API endpoints the page calls (`/api/hive-graph/search`, `/status`, `/build`, `/projection`) — **PRD-008**. This PRD consumes them; the file graph is a client-side transform of PRD-008c's existing projection read (`GET /api/hive-graph/projection`), not a new endpoint (decision #39).
...
- A 3rd graph on `/graph`. The density-failure lesson (`registry.tsx:215-218`) rules this out; the Hive Graph is a separate route.
...
| ID | Criterion |
|---|---|
| AC-1 | Given hive is running and serving the shared `ROUTES` list (`registry.tsx:204-231`), when the operator navigates to `/hive-graph`, then the `HiveGraphPage` mounts via `matchRoute` (`registry.tsx:243-253`) and a "Hive Graph" nav item appears in the sidebar — added by ONE `RouteEntry`, with no sidebar or router hand-edit. |
| AC-2 | Given `HiveGraphPage` takes the shared `PageProps` (`registry.tsx:92`), when it mounts, then it hydrates via the shared `wire` + `usePoll` recipe (fetch-on-mount + interval + cleanup-on-unmount), mirroring `GraphPage` (`graph.tsx:435-482`). |
| AC-3 | Given the file-graph panel, when it renders, then each node is a nectar (a file) and each edge is a `derived_from_nectar` provenance link — and the page reuses the existing `GraphWire` node/edge shape (`wire.ts:199-208`) so the pan/zoom/select interaction is shared with the memory graph. |
...
| AC-6 | Given nectar is down but hive is up, when the operator opens `/hive-graph`, then hive renders the shell + page chrome with the source marked unreachable, rather than blanking the dashboard (c-AC-2 from [PRD-004c](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md)). |
| AC-7 | Given the codebase-graph density failure (`registry.tsx:215-218`), when the Hive Graph is added, then it is a NEW route (`/hive-graph`), NOT a third graph on `/graph`; the `/graph` page still renders only the memory graph. |
...
|---|---|---|
| Route | `/hive-graph` | A real, server-served path matching the registry convention (`/graph`, `/memories`, `/sync`; path-based routing per PRD-003c). Flagged so an operator who prefers `/files` or `/source` changes only this value. |
| Page label | `Hive Graph` | The nav label + per-route document title source (`registry.tsx:87-88, OQ-2`). Flagged so it can be tightened (e.g. `Files`, `File Graph`). |
| Icon | **DEFAULT — pick from hive's icon set at implementation.** | Icons are inline-SVG `ReactNode`s stroked in `currentColor` (`registry.tsx:89-90, 98-104`), matching the no-new-dependency posture. The specific glyph (e.g. a file-tree or git-fork shape) is an implementation-time pick from the existing inline-SVG idiom, not a decision this PRD pins. |
...
- [`prd-004c-hive-portal-daemon`](../../completed/prd-004-doctor-registry-and-hive/prd-004c-hive-portal-daemon.md) — owns hive (the always-on portal that hosts this page) + its API-aggregation `wire`.
- [`prd-001-three-daemon-topology`](../../completed/prd-001-three-daemon-topology/prd-001-three-daemon-topology-index.md) — the three-daemon topology; hive serves on port 3853 (locked port contract).
...
- [`prd-012-manual-hive-graph-search`](../../in-work/prd-012-manual-hive-graph-search/prd-012-manual-hive-graph-search-index.md) — owns the manual-search query semantics the search box consumes. *(PRD-012 is authored alongside this index.)*
- [`prd-005-hive-graph-catalog-tables`](../../completed/prd-005-hive-graph-catalog-tables/prd-005-hive-graph-catalog-tables-index.md) — owns the `hive_graph` table + the `derived_from_nectar` column the file-graph edges read.
...
- `honeycomb/library/knowledge/private/dashboard/adding-a-page.md` — the documented "how to add a page" procedure (referenced at `registry.tsx:21`).
