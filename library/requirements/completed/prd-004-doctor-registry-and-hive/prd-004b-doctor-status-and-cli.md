# PRD-004b: doctor status page + CLI — multi-daemon reporting

> **Codebase:** the `doctor` repo. This is an out-of-band sub-PRD; it lands in the doctor codebase, not nectar.

## Overview

Once 004a spawns N supervisor instances over isolated state shards, doctor's two operator-facing reporting surfaces must report all N daemons, not just one. Today the loopback status page (`doctor/src/status-page/server.ts`) serves a single coarse `health` + a single `escalation` (`server.ts:40-48`, `197-206`), and the CLI `status` command (`doctor/src/cli/dispatch.ts:58-87`) probes one daemon and prints one health line. This sub-PRD generalizes both to iterate the registry: the status page renders a per-daemon table, and the CLI prints one block per registered daemon.

These surfaces remain doctor's own read-only supervision views. They are distinct from hive's unified workload dashboard (004c): doctor's page is "what is the supervisor doing to each daemon"; hive's is "what is each workload showing me."
...
- No hive dashboard work here. hive's portal is 004c.
- No new CLI commands beyond extending `status`/`diagnose`/`logs` to iterate the registry. The command surface (`doctor/src/cli/command-table.ts`) is unchanged.
- No live registry mutation from the CLI. The CLI reads the registry; it does not edit it (004d owns registry edits).
- No remote/network exposure. The status page stays bound to `127.0.0.1` (`doctor/src/status-page/server.ts:96`).
...
**As** an operator, **when** I open doctor's loopback status page, **I** see every registered daemon's health at once.
...
| b-AC-3 | Given the HTML page (`/`), when rendered, then it shows one row/badge per registered daemon with its health, mirroring the existing per-health badge styling in `doctor/src/status-page/server.ts:144-184`. |

### US-2 — Multi-daemon CLI status

**As** an operator, **when** I run `doctor status`, **I** get a block per registered daemon.

| ID | Criterion |
|---|---|
| b-AC-4 | Given a registry with three daemons, when `doctor status` runs, then it prints one block per daemon, each with its own health label, version, last-heal, and opt-out line — the per-daemon analogue of the single block in `doctor/src/cli/dispatch.ts:71-85`. |
| b-AC-5 | Given `doctor status`, when a registered daemon is unreachable, then its block reports `unreachable` (mirroring `dispatch.ts:37-50`'s `healthLabel`) without crashing the command or aborting the other blocks. |
...
| ID | Criterion |
|---|---|
| b-AC-6 | Given `doctor logs --daemon nectar`, when run, then it tails only the nectar incident stream (004a's `incidents-nectar.ndjson`), not every daemon's. |
| b-AC-7 | Given `doctor logs` with no `--daemon` flag, when run, then it tails every registered daemon's stream interleaved with a daemon-name prefix per line. |
...
Today `createStatusPageServer` takes a single `StatusPageStateProvider` with `health()` and `escalation()` (`doctor/src/status-page/server.ts:50-56`), and the composition root wires it over one `stateStore`/`needsAttention` pair (`doctor/src/compose/index.ts:378-389`). The generalization: the state provider iterates the registry. Concretely the root passes a provider whose `health()`/`escalation()` (or a new `entries()` method) reads each supervisor's isolated state shard from 004a and returns the per-daemon array b-AC-1 requires.
