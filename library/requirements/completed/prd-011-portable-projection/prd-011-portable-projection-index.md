...
- `honeycomb nectar rebuild-projection` and `honeycomb nectar project --rebuild-projection` regenerate a byte-identical file (modulo `generated_at`) from a Deep Lake scan alone, enforcing the regenerable-from-source invariant.
...
| AC-3 | Given the operator runs `honeycomb nectar rebuild-projection`, then the file is regenerated from a single `hive_graph_versions` scan (latest described version per nectar, scoped to the project) and written atomically. |
...
None. The projection is a committed file under the project root, not a Deep Lake table. It denormalizes the latest described version per nectar from the `hive_graph` / `hive_graph_versions` tables owned by PRD-005 (`data/hive-graph-schema.md`).

---

## API changes

None. The projection is generated and loaded internally. The two CLI commands (`rebuild-projection`, `project --rebuild-projection`) extend the `nectar` CLI surface owned by PRD-002c (spec'd CLI surface, `MASTER-PRD-INDEX.md`).
...
- [`../../../knowledge/private/data/hive-graph-schema.md`](../../../knowledge/private/data/hive-graph-schema.md) — the `hive_graph` / `hive_graph_versions` tables the projection denormalizes.
