...
Expose hivenectar's non-sensitive operational metrics in its own local SQLite so hivedoctor can poll them, per `ADR-0001-hive-telemetry-transport-and-single-source-of-truth.md`. Emit the nectar-pipeline counters since the last restart: files registered, nectars minted, descriptions generated, source-graph versions written, and embeddings computed.

## Scope

- A `hivenectar_metrics` table in hivenectar's local SQLite (Node's built-in `node:sqlite`), holding a latest-wins snapshot of since-restart counters.
- The hivenectar metric set: files registered (PRD-006 intake), nectars minted (ULID identity), descriptions generated (the enricher and brooding loop, PRD-007 and PRD-016), source-graph versions written (`source_graph_versions`, PRD-005), and embeddings computed (PRD-014).
...
### US-017b.1 - hivedoctor can poll live metrics

**As** hivedoctor, **I want** hivenectar's metrics in local SQLite, **so that** I can read them on my interval without hivenectar pushing.

- AC-017b.1.1 Given hivenectar is doing pipeline work, when hivedoctor reads `hivenectar_metrics`, then it sees current values for files registered, nectars minted, descriptions generated, source-graph versions written, and embeddings computed since restart.
...
- AC-017b.2.1 Given the nectar pipeline stages, when the metrics snapshot is written, then each counter (files registered, nectars minted, descriptions generated, source-graph versions, embeddings computed) is incremented once per unit of work, without double counting.
...
- AC-017b.3.1 Given a hivenectar restart, when the metrics snapshot is next read, then the since-restart counters reflect the new process lifetime starting from zero.
...
- The write goes through Node's built-in `node:sqlite` in WAL mode so hivedoctor's read-only open does not contend with hivenectar's writes (per `ADR-0001-hive-telemetry-transport-and-single-source-of-truth.md`).
...
- Integration: a read-only reader observes live values while hivenectar writes (AC-017b.1.1).
...
- [ ] Is "files registered" counted per file or per intake batch on hivenectar's registration path?
