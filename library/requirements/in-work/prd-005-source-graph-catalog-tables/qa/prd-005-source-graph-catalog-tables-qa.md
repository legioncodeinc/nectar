# QA Report: PRD-005 Source Graph Catalog Tables (PRD-vs-Corpus Conformance)

> Category: QA Report | Version: 1.0 | Date: July 2026 | Status: Active

Conformance audit of PRD-005 (index + 005a/005b/005c) against the Hivenectar knowledge corpus and the cited Honeycomb code, armed with quality-stinger + hivenectar-stinger. This is a PRD-vs-corpus/code pass (no implementation exists yet), matching the bar and format of the consolidated PRD-001-004 report and the per-module prd-002/prd-003 reports. Every acceptance criterion and load-bearing claim was traced to `data/source-graph-schema.md` (authoritative DDL), `MASTER-PRD-INDEX.md` decision #3, ADR-0001/ADR-0002, and the real files under `honeycomb/src/daemon/storage/`.

**Related:**
- [`prd-005-source-graph-catalog-tables-index.md`](../prd-005-source-graph-catalog-tables-index.md)
...
- `prd-005b:182` (AC): "`SOURCE_GRAPH_VERSIONS_COLUMNS` has exactly twenty entries." The array has twenty-one entries, so this AC as written is not satisfiable by the correct array.
...
Four links to the decisions ledger use `../../../MASTER-PRD-INDEX.md`. From a file in `in-work/prd-005-source-graph-catalog-tables/`, `../../../` resolves to `library/`, so the target is `library/MASTER-PRD-INDEX.md`, which does not exist. The file is at `library/requirements/MASTER-PRD-INDEX.md`, so the correct path is `../../MASTER-PRD-INDEX.md` (up to `requirements/`). Note the same files correctly use three levels for `../../../knowledge/...` (which does resolve, since `knowledge/` is under `library/`), so this is a per-target depth error, not a whole-file base error.

Locations:
- `prd-005-source-graph-catalog-tables-index.md:9`
- `prd-005-source-graph-catalog-tables-index.md:65`
...
The PRD body cites honeycomb code with canonical backtick file-path spans (for example `prd-005-...-index.md:13` uses `honeycomb/src/daemon/storage/client.ts:40-46` as a span). The **Related** sections diverge: they wrap the span in a markdown link whose target is `../../../../honeycomb/...`. From `in-work/prd-005-.../`, `../../../../` resolves to `library/`, so the target is `hivenectar/library/honeycomb/...`, which does not exist. This is the systemic W-1 finding from the consolidated report (Documentation Framework section 6 and `AGENTS.md` require cross-repo code to be a backtick file-path span, not a markdown link).

Link-form honeycomb token counts (all `honeycomb/src/...`; zero `hivedoctor/...`, consistent with the consolidated report's note that PRD-005 carries no hivedoctor references):

| File | Link-form honeycomb tokens | Lines |
|---|---|---|
| `prd-005-...-index.md` | 6 | 66, 67, 68, 69, 70, 71 |
| `prd-005a-source-graph-table.md` | 4 | 129, 130, 131, 132 |
| `prd-005b-source-graph-versions-table.md` | 4 | 195, 196, 197, 198 |
...
- **N-1 (informational, C-2 disposition update):** The consolidated report tracked C-2 as two known corpus/PRD disagreements (the `confidence` column and the `skipped-deleted` enum value). The corpus `data/source-graph-schema.md` now already carries both: `confidence REAL` at `source-graph-schema.md:78` (with rationale at `:104`) and `skipped-deleted` at `source-graph-schema.md:81` and `:107`. PRD-005b's DDL matches the corpus verbatim, so **there is no longer a disagreement to record**; the corpus-side edit is effectively complete. The only residue is that PRD-005b's prose still reads as if the corpus lacks these: the blockquote at `prd-005b:60` says "The corpus's `source-graph-schema.md` should be updated to match (additive schema change)" and both `:60` and `:84` label the columns "(added per QA finding)." These notes are now stale. Sub-medium; consider softening to past tense ("carried in the corpus DDL as of the July 2026 revision") so the note documents history rather than implying a pending corpus edit. Per audit scope, the corpus was not modified.
- **N-2 (Alignment, descriptive inaccuracy, 005a):** `prd-005a:85` states that `SOURCE_GRAPH_COLUMNS` "mirrors the structure of `CODEBASE_COLUMNS` ... tenant-identity columns first, row identity next." `CODEBASE_COLUMNS` does place `org_id`/`workspace_id` first (`product.ts:216-219`), but `SOURCE_GRAPH_COLUMNS` places row identity (`nectar`, `kind`, `created_at`) first and the tenancy columns later (`prd-005a:68-82`). The ordering claim is inaccurate for the array it describes. No functional impact (column order does not affect the load-time guard or catalog correctness). Sub-medium; consider rewording to "mirrors the tenant-scoped `CODEBASE_COLUMNS` convention (explicit `org_id`/`workspace_id`/`project_id`, `as const` + `Object.freeze`)" without asserting a specific column order.
...
|---|---|---|
| Both DDL blocks verbatim; every column name and SQL type cross-checks | `source-graph-schema.md:32-43` (source_graph), `:65-87` (versions) | PASS (both blocks match character-for-character, incl. `confidence` and `skipped-deleted`) |
...
| `source-graph` group appended to `CATALOG`; `REGISTRY = buildRegistry(CATALOG)` picks up patterns | `catalog/index.ts:45-59`, `:62` | PASS (planned edit; both sites verified; group name is a preserved DEFAULT) |
...
### PRD-005a source_graph (6 ACs)

| AC (005a) | Source | Verdict |
|---|---|---|
| Verbatim DDL matches source (nine columns, all TEXT NOT NULL DEFAULT) | `source-graph-schema.md:32-43` | PASS |
| `SOURCE_GRAPH_COLUMNS` has exactly nine entries in declaration order | `prd-005a:68-82` | PASS (9 entries) |
| Array passes `validateColumnDefs("SOURCE_GRAPH", ...)` | `schema.ts:80-100` | PASS |
| `CatalogTable` declares `scope: tenant`, `embeddingColumns: []`, write pattern | `types.ts:80-95`; pattern is a preserved DEFAULT | PASS |
| Record spread into `source-graph` group appended to `CATALOG` | `catalog/index.ts:45-59` | PASS (planned edit) |
| First write → one `buildCreateTableSql` + one retry in `withHeal` | `heal.ts:286-313` | PASS |

### PRD-005b source_graph_versions (7 ACs)

| AC (005b) | Source | Verdict |
|---|---|---|
| Verbatim DDL matches source | `source-graph-schema.md:65-87` | PASS on the verbatim match; the parenthetical "twenty columns" count is wrong (see W-1) |
| `SOURCE_GRAPH_VERSIONS_COLUMNS` has exactly twenty entries incl. `embedding` `"FLOAT4[]"` | `prd-005b:96-126` | FAIL as written (array has 21 entries; array is correct, AC count is wrong) - see W-1 |
...
| First write → one `buildCreateTableSql` + one retry in `withHeal` | `heal.ts:286-313` | PASS |
| `embedding` documented 768-dim `FLOAT4[]`, nullable, matches `sessions`/`memory` embeddings | `source-graph-schema.md:103`; ADR-0001 (768-dim tied to schema) | PASS |

### PRD-005c tenancy + project_id soft-filter (6 ACs)

| AC (005c) | Source | Verdict |
|---|---|---|
| Both tables `scope: tenant`; carry `org_id`/`workspace_id`/`project_id` as `TEXT NOT NULL DEFAULT ''` | `prd-005a`, `prd-005b`; `source-graph-schema.md` tenancy section | PASS |
...
Confirmed present and intentional, not defects: the catalog group name `source-graph` (DEFAULT), the `update-or-insert` (005a) and `append-only` (005b) write patterns (DEFAULT), and `CatalogScope: tenant` for both tables (DEFAULT), each carrying "DEFAULT - confirm before implementation." Symbol-level and directory-level nectars are a stated v1 non-goal (`kind` reserves the `'directory'` namespace). The additive `confidence REAL` column and the `skipped-deleted` `describe_status` value are self-declared documented additive changes that now match the corpus verbatim; they are not treated as defects.

## 8. High-risk surfaces verified verbatim / against source

- `source_graph` DDL: 9 columns, all `TEXT NOT NULL DEFAULT`, matches `source-graph-schema.md:32-43`.
- `source_graph_versions` DDL: 21 columns (19 `NOT NULL DEFAULT`, 2 nullable: `embedding FLOAT4[]`, `confidence REAL`), matches `source-graph-schema.md:65-87`.
...
- `prd-005-source-graph-catalog-tables-index.md` - audited (carries W-2, W-3). (audited)
- `prd-005a-source-graph-table.md` - audited (carries W-3; note N-2). (audited)
- `prd-005b-source-graph-versions-table.md` - audited (carries W-1, W-3; note N-1). (audited)
...
**Verification:** `grep` for `](.../honeycomb` and `](.../hivedoctor` markdown-link tokens across the PRD-005 folder returns zero (excluding the QA report's own descriptive text and the legitimate ADR-0002 knowledge-doc link whose filename contains "hivedoctor"); `grep` for `../../../MASTER-PRD-INDEX` returns zero in the PRD files; the column-count prose and ACs are internally consistent with the 21-column artifact. All PRD-005 acceptance criteria now verify PASS with no medium-or-above finding open.
