...
- **Usage accounting.** The transport feeds the `UsageSink` on the success path, mapping `prompt_tokens → inputTokens`, `completion_tokens → outputTokens`, and `prompt_tokens_details.cached_tokens → cacheReadInputTokens` (`transport-portkey.ts:271-279`). A missing/malformed `usage` surfaces zero counts rather than throwing (`transport-portkey.ts:183`; schema field guards at `:165-172`). This is the seam that accounts for upstream cached tokens; it does not enable caching.
