...
Two Deep Lake tables implement the identity+version split that makes Nectar's stable identity possible. `hive_graph` is one row per logical file, keyed by an immutable ULID nectar, carrying provenance and tenancy and nothing else. `hive_graph_versions` is append-only, keyed by `(nectar, content_hash)`, carrying every observed state of every file plus the lazily-filled title, description, concepts, and embedding. The two tables together answer the two questions Nectar exists to answer cheaply: *"what is the current state of file X"* is the latest version row for its nectar; *"what is the full history of file X"* is all the version rows for its nectar. Both are single-table queries against an append-only chain.
...
|---|---|---|
| **Nectar is immutable** | `hive_graph.nectar` is set once at minting and never recomputed, re-derived, or reused. | Identity churns per save; history chains corrupt. |
| **Versions are append-only** | Edits append a new `hive_graph_versions` row keyed by the new `content_hash`; previous rows are never overwritten. | History is lost; "what did this file look like last week" becomes unanswerable. |
...
1. **No hand-rolled `ALTER`.** Additive schema changes go through `withHeal` against the canonical `ColumnDef` array in the daemon's schema module. A manual `ALTER TABLE` against `hive_graph` or `hive_graph_versions` desynchronizes the heal pass and produces drift. See the lazy-schema-heal section of [`hive-graph-schema.md`](../hive-graph-schema.md).
...
## Composition with the rest of Nectar

The schema is the substrate four other subsystems read and write:

```mermaid
flowchart LR
    mint["hiveantennae: mint nectar"] --> sg[("hive_graph")]
    edit["watcher: edit detected"] --> sgv[("hive_graph_versions")]
...
- **Minting** ([`../../ai/identity-and-reassociation.md`](../../ai/identity-and-reassociation.md)) writes the `hive_graph` row and the initial `hive_graph_versions` row.
- **The enricher** ([`../../ai/enricher-and-llm-model.md`](../../ai/enricher-and-llm-model.md)) updates `hive_graph_versions` rows from `describe_status = 'pending'` to `'described'`, filling the title/description/embedding columns.
...
| If you need... | Read... |
|---|---|
| The why behind the two-table split (the conceptual argument) | [`hive-graph-introduction-and-theory.md`](hive-graph-introduction-and-theory.md) |
| The full DDL, column tables, indexing strategy, heal rule | [`hive-graph-technical-specification.md`](hive-graph-technical-specification.md) |
| How writes and reads flow through the tables end-to-end | [`hive-graph-ecosystem-story-arc.md`](hive-graph-ecosystem-story-arc.md) |
| The operator/engineering scope and acceptance criteria | [`hive-graph-user-stories.md`](hive-graph-user-stories.md) |
...
The schema is the load-bearing substrate. Every other Nectar component is a producer or consumer of these two tables.
