...
The deliverable of the recall-integration work restated in one place — a fourth recall arm that fuses the CodeGraph's structural answers and Nectar's semantic answers into one ranked list — alongside the two contracts (complementarity and graceful degradation) the arm honors, and the forward pointers to the rows, the descriptions, and the two-layer thesis that motivate the design.
...
The arm rests on a complementarity contract between the structural CodeGraph and the semantic Nectar layer. The contract has two clauses, and the arm is only useful because both hold.
...
The vector path gates on `embedding IS NOT NULL`. When the configured embedding provider is unavailable, the `embedding` column is NULL across the arm, the vector path returns nothing, and the BM25 path carries recall alone over `title` and `description`. There is no error, no empty result, and no special-case configuration. The arm returns lexical hits ranked by BM25 until embeddings are available, at which point vector scoring resumes on the next provider warm-up without a manual re-index step.

This is the same silent-fallback behavior every other recall arm in Honeycomb uses. The contract is that Nectar is not a special case operationally: disabling embeddings degrades the arm the same way it degrades sessions, memory, and memories — lexical-only, no cliff. An operator who runs Nectar without an available embedding provider gets working semantic recall over file descriptions; an operator who enables local nomic or hosted Cohere via Portkey gets hybrid lexical+vector recall over the same descriptions. The arm is useful in both states.
...
- **It does not deduplicate against CodeGraph hits.** A file appearing in both a Nectar hit and a structural hit is returned twice; reconciliation is the agent's job.
...
The canonical single-document summary remains [`../recall-integration.md`](../recall-integration.md); this deep-dive expands it along five axes without superseding it. The arm ships when its SQL contract is implemented behind the storage-layer guards, its acceptance criteria pass, and the two contracts (complementarity, graceful degradation) hold under test — at which point the CodeGraph's structural answers and Nectar's semantic answers appear in one fused ranked list, and the agent no longer has to run two queries and reconcile them in its own head.
