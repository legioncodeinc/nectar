...
This is the same silent-fallback behavior every other recall arm in Honeycomb uses. The contract is that Hivenectar is not a special case operationally: disabling embeddings degrades the arm the same way it degrades sessions, memory, and memories — lexical-only, no cliff. An operator who runs Hivenectar without an available embedding provider gets working semantic recall over file descriptions; an operator who enables local nomic or hosted Cohere via Portkey gets hybrid lexical+vector recall over the same descriptions. The arm is useful in both states.
