...
**Cold catch-up after offline changes is the hard case.** During live operation, the chokidar watcher carries move semantics and step 3 handles moves directly. Cold catch-up — the daemon boots after the laptop was closed while files were moved and edited — relies on steps 3 and 4, and may surface low-confidence matches for human review. This is acceptable because cold catch-up is rare and the ladder is conservative: a mis-association is worse than a new nectar, so low-confidence candidates are surfaced rather than auto-claimed.
