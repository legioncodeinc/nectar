...
These stories are scoped to the people who *operate and build* Nectar, not end-product users. The five personas:

- **The daemon operator** — runs `nectar daemon` (registered with doctor per ADR-0003), configures the model provider, watches cost and queue depth in hive's dashboard.
...
**US-OV-010** — As a daemon operator, I want the description model to be configurable and auditable, so that I can swap models without code changes and know which model produced each description. **Acceptance criteria:** (a) Gemini 2.5 Flash is the default in the provider router, swappable via the same configuration as every other LLM call; (b) every `hive_graph_versions` row records its `describe_model`; (c) a model swap does not automatically re-describe existing rows — that requires an explicit `brood --force --model`.
...
**US-OV-023** — As a daemon operator, I want the Nectar daemon (`hiveantennae`) to be an independent workload process registered with doctor and surfaced through hive, so that it has an isolated failure domain, independent release cadence, and an always-on portal surface (per ADR-0002 and ADR-0003). **Acceptance criteria:** (a) the daemon runs as its own OS process under doctor supervision, restartable independently of Honeycomb; (b) doctor has a registry entry for Nectar with health URL, PID path, and probe settings; (c) Nectar obtains its own Deep Lake client pointed at the same org/workspace datasets Honeycomb recall reads, with `project_id` applied as a column filter; (d) enricher cycles log files described, inherited, failed, tokens consumed, and estimated cost for hive to surface through Nectar's API.
