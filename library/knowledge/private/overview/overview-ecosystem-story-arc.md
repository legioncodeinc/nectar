...
Honeycomb already had a structural graph that knew how code was wired; it lacked a semantic layer that knew what code was for; Nectar is that layer, and the two are united by guarded recall fusion so an agent gets both signals at once — and the whole thing travels to teammates through a committed lockfile.
...
|---|---|---|
| **CodeGraph** | The structural layer; answers symbol-shaped queries; coexists with Nectar in disjoint tables. | `data/codebase-graph.md` (main corpus) |
| **Recall pipeline** | The hybrid BM25 + vector arm set, fused by RRF, that Nectar joins as a fourth guarded arm. | [`../data/recall-integration.md`](../data/recall-integration.md); main corpus `ai/retrieval.md`, `ai/hybrid-sql-vector-rationale.md` |
...
| **Daemon lifecycle** | hiveantennae is an independent Nectar workload daemon registered with doctor; brooding does not block readiness. | [`overview-technical-specification.md`](overview-technical-specification.md); [`../architecture/ADR-0003-three-daemon-topology-and-hive-portal.md`](../architecture/ADR-0003-three-daemon-topology-and-hive-portal.md) |
| **hive portal** | Hosts the always-on dashboard and Hive Graph page, fetching data from Nectar and Honeycomb APIs. | [`../architecture/ADR-0003-three-daemon-topology-and-hive-portal.md`](../architecture/ADR-0003-three-daemon-topology-and-hive-portal.md) |
| **Deep Lake substrate** | The only durable store; `hive_graph` and `hive_graph_versions` are Deep Lake tables subject to the same FR-8 rule and schema-heal pass as the rest. | [`../data/hive-graph-schema.md`](../data/hive-graph-schema.md) |
...
Read left to right: the source tree feeds both layers; the CodeGraph alone leaves the meaning-shaped query blind; Nectar adds the semantic arm; recall unites them; the result is committed and inherited on clone.
...
sequenceDiagram
    ...
    and semantic arm (BM25)
        recall->>dl: ILIKE over hive_graph_versions title/description
...
|---|---|---|
| `src/auth/login.ts` | CodeGraph `find/login` + Nectar | The named entry point; *and* "validates credentials, starts a session, issues a JWT refresh token" |
| `src/middleware/session-refresh.ts` | Nectar only | "Refreshes JWT claims on each authenticated request, part of the login session lifecycle" — invisible to `find/login` because no symbol is named `login*` |
| `src/lib/jwt.ts` | Nectar only | "JWT issue/verify, used by login and session-refresh" |
| `src/api/routes/logout.ts` | Nectar only | "Ends a login session, clears refresh token" |
...
The arc is deliberately bounded. Nectar composes with the subsystems above; it does not absorb them, and it does not extend past them. Specifically:
...
- It does not own embedding infrastructure. The provider stack does; Nectar only requires the 768-dim dimensionality to match the other recall tables so one index serves all.
- It does not own the model gateway. Portkey does; Nectar specifies the default model and routes through Portkey, while semantic caching and guardrails are configured server-side.
- It does not own the portal. hive does; Nectar exposes APIs and status that the portal consumes.
...
The arc shows *how* the pieces fit. For *what* Nectar delivers as concrete outcomes and how to measure success, read [`overview-conclusion-and-deliverables.md`](overview-conclusion-and-deliverables.md). For the operating contract that makes each beat work, read [`overview-technical-specification.md`](overview-technical-specification.md).
