...
**US-BR-001** — As an operator running Hivenectar against a fresh project, I want brooding to begin automatically the first time the daemon sees no `source_graph` rows (and no `.honeycomb/nectars.json`), so that I get a semantic index without an explicit command.
...
**US-BR-003** — As the brooder, I want to discover files by reusing the CodeGraph's `git ls-files --cached --others --exclude-standard -z` logic (with manual recursive walk fallback when git is unavailable), so that Hivenectar and the CodeGraph never disagree on what counts as a file in the repo.

**Acceptance criteria:** (a) Discovery uses the same command and the same `~/.honeycomb/graph-ignore.json` per-repo ignore file as the CodeGraph. (b) Hivenectar maintains no separate ignore list. (c) A file in the CodeGraph's discovery set is in Hivenectar's; a file not in it is not.
...
**Acceptance criteria:** (a) A file matching the binary criteria gets a minted nectar. (b) Its `source_graph_versions` row has `describe_status = 'skipped-binary'`, `title = filename`, `description = ''`. (c) No LLM call is made for the file.
...
**Acceptance criteria:** (a) A file with `size_bytes > 256 KB` gets a minted nectar. (b) Its row has `describe_status = 'skipped-too-large'`, `title = filename`. (c) The structural CodeGraph still extracts symbols from the file if it is source; Hivenectar simply does not describe it semantically.
...
**Acceptance criteria:** (a) The request body is a JSON array of objects with `nectar`, `path`, `content`. (b) The system prompt matches the verbatim batch prompt. (c) The response is parsed as a JSON array and validated for per-entry shape and array length. (d) Descriptions are written to the corresponding `source_graph_versions` rows with `describe_status = 'described'`.
...
**US-BR-018** — As an operator, I want `honeycomb hivenectar brood --dry-run` to run discovery and bucketing, print the estimated call count and cost, and exit without LLM calls, so that I can sanity-check the cost of a brood before committing to it.
...
**US-BR-019** — As an operator, I want `honeycomb hivenectar brood --limit N` to cap the number of pending files processed in one invocation, so that I can bound the cost of an explicit brood.
...
**US-BR-020** — As an operator, I want `honeycomb hivenectar brood --force` to re-describe every non-skipped file (ignoring existing descriptions), so that I can re-brood after a model swap or a quality regression.
...
**US-BR-021** — As an operator, I want a plain `honeycomb hivenectar brood` to respect existing descriptions and only process pending files, so that re-running an interrupted or `--limit`-capped brood fills in the gaps without redoing finished work.
...
**Acceptance criteria:** (a) Brooding does not skip files the CodeGraph covers structurally. (b) A source file is present in both `source_graph_versions` (semantic) and the CodeGraph's `codebase` table (structural). (c) The two workers may run concurrently against the same file without coordination because they write to different tables.
