...
Nectar's hiveantennae daemon has four operating modes — brooding, live watch, cold catch-up, and projection sync — and brooding is the only one that runs against the entire codebase at once. The distinction is not a matter of scope; it is a matter of *what the daemon can assume*.
...
The naïve approach to "a long-running scan that might be interrupted" is a lockfile or a progress marker: write "brood in progress, checkpoint at file N" to a sidecar, update it as you go, clean it up when done. Nectar rejects this. There is no brood-in-progress lockfile, no partial-state marker, no checkpoint file. The reason is that the state of a brood is *fully derivable* from data that is already being written: the `describe_status` column on every `hive_graph_versions` row.
