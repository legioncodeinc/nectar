...
Explains how Hivenectar keeps every file's description current as you edit, rename, move, and copy-paste — without re-describing on every keystroke, and without losing track of a file when it moves.

**Related:**
- [`getting-started-with-hivenectar.md`](getting-started-with-hivenectar.md)
...
After the [first scan](getting-started-with-hivenectar.md), your project has a description for every file. But code is not static. You edit files, rename them, move them between folders, copy blocks from one place to another, and delete things. If the descriptions stayed frozen at their first-scan wording, they would drift out of sync with reality within a day.

Hivenectar's job in steady state is to keep descriptions accurate **without hovering over your shoulder**. It does this with four behaviors, each tuned to a specific kind of change. The guiding principle throughout: only re-describe when something has *meaningfully* changed, so cost stays low and your descriptions stay trustworthy.

---

## Edits — descriptions update after a pause, not on every keystroke

When you save a file, Hivenectar notices. But it does not rush to re-describe it the instant you press save — and it certainly does not re-describe on every keystroke. That would be wasteful, jumpy, and expensive.

Instead, it waits through a short pause. If you save the same file several times in quick succession (as you almost always do while working), those saves collapse into a single "this file changed" signal. Only after you have stopped editing for a moment does Hivenectar take a closer look.
...
Only when the change crosses a meaningful threshold does Hivenectar write a fresh description. The result is that routine editing costs essentially nothing, and descriptions only change when they genuinely need to.
...
Hivenectar does not work that way. It gives each file a stable identity that is **independent of its name or location**. When you rename a file or move it to a different folder, Hivenectar recognizes that it is the same file in a new place. The description travels with it.

In practice this means:

- **Rename `login.ts` to `auth-handler.ts`** — the description stays attached. You do not lose the understanding Hivenectar built, and you do not pay to re-build it.
...
This is one of the most important properties of Hivenectar: **refactoring does not reset understanding.** You are free to rename and reorganize as much as you like.
...
Hivenectar handles this by giving the copy a fresh identity **and a link back to the original**. The copy keeps a pointer that says "I came from here." This is useful in two ways:
...
Most of the time, **the problem fixes itself.** The next time you meaningfully edit that file, Hivenectar re-describes it from scratch, and the fresh description is often clearer than the original. Patience is a valid strategy.

When a description seems consistently wrong — especially right after the first scan, or for a file whose purpose is genuinely ambiguous — you can ask Hivenectar to take a second look. The command for low-confidence or suspicious cases is:

```bash
honeycomb hivenectar review-matches
```

This surfaces the files Hivenectar was least sure about — the ones where its tracking or description confidence was low — so you can see what it decided and, where needed, prompt it to reconsider. It is the right tool when something feels off and you want to nudge the system rather than wait for the next natural re-description.
...
It is worth restating the reassurance, because "AI describes your files" can sound expensive. The first scan is the only time Hivenectar describes everything at once, and even that is a small one-time cost (see the [getting started guide](getting-started-with-hivenectar.md)).

After that, Hivenectar re-describes a file only when **all** of these are true:
...
- **Wrong descriptions** usually self-correct on the next meaningful edit; for stubborn low-confidence cases, use `honeycomb hivenectar review-matches`.
