...
- 2026-07-03 08:35 PR #18 review remediation: all 8 CodeRabbit findings of Major/Critical severity fixed with 17 new regression tests (no Aikido comments existed on the PR). Fixes: the reused-instance start() closed-flag wedge (Critical); corrupt-lock reclaim + lock-body write rollback; duplicate-column ALTER race swallow; stale advisory-lock mtime reclaim; fail-closed case probe; review prompt now prints its preview; embedModelId threaded through the CLI brood path; orphaned-version prevention in the store bridge plus orphan-version repair in the resync sweep. Orchestrator follow-up: live-gate skip discipline refined once more, 5xx backend errors (server-side unreachability, e.g. 'failed to get database connection') may skip like connection/timeout, while 4xx query errors still fail the gate (a live-backend 500 blip flaked two runs). Suite: 664 tests, 661 pass, 0 fail, 3 platform skips; both live gates passing. Committed and pushed.
